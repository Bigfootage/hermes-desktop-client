import { HermesClient } from './client'; import { parseSse } from './sse'; import type { HermesApprovalChoice, HermesRun, HermesRunEvent } from './types';
export class HermesRunsClient {
  ...
  approve(id: string, choice: HermesApprovalChoice, all?: boolean) { return this.client.request<HermesRun>(`/v1/runs/${encodeURIComponent(id)}/approval`, { method: 'POST', body: JSON.stringify({ choice, ...(all !== undefined ? { all } : {}) }) }); }
